...
def main():
    ...
    print("Starting locator thread...")
    threading.Thread(target=locator, daemon=True).start()
    
    threading.Thread(target=run_motor_info_server,
    args=(stop_event,),
    kwargs={"on_message": handle_ints, "period": 0.5},
    daemon=True).start()
